...
### Classes and objects
The fundamental concept of every object oriented language is the idea of classes and objects. A class is a description of data and behaviour (== methods). So it wraps functions and data into one structure. Once this is defined, one create as many instances of this class as are necessary. Instances of classes are called objects. Given the example below, once you have defined which data and functions are necessary to describe a sport club, you can create an object of this class for every real world club, that you want to manage.
...
* Core idea is that underneath an exposed set of functions (aka API) a specific implementation can be exchanged without breaking existing code using that API.
...
* However, it must be used with care, because premature abstraction can make it hard to change the code later on.
* __Note__ This concept dominates software development and especially in Java it enables you, to write highly reusable code.

### Inheritance
* The idea is to derive similar classes from a common super type sharing existing functionality, which helps in reusing code.
* Looking at the car example - every car has (at least) four wheels, a number of seats, a color,... its engine can be turned on/off, lights can be turned on/off. So this would be data and functions for a base class and now you can create classes for passenger cars, commercial vehicles, busses, ... Each of which extends or modify the base class.
* Add small changes to a shared set of data/functions
* Software projects usually have a sophisticated hierachy of classes with inheritance. 
...
* In inheritence hierachies classes can not only add, but also modify behavior. 
* In the car example, each of the inheriting classes could use its own function to model acceleration. So every class has the same functionality, whereas it's implementation can be very different.
...
Core idea here is that a football club __is a__ sports club and thus inherits all attributes. Same goes for sailing clubs or any other kind of sports club one could think of. All of those will have a name, a founding date and a number of members.
...
In this course we will use DrawIO to create UML diagrams. It is a free tool and you can use it in a number of ways:
